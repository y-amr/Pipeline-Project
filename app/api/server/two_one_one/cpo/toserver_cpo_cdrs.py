...
@ocpi_cpo_211.route("/cdrs", methods=['GET'])
@requires_auth_api
def toserver_get_cpo_cdrs():
    # Init result table
    result_tab = []
    ...
    iop_ws = IOPWebserviceService.get_iop_webservice_by_ocpiversion_name_type(ocpiversion=OCPIVersion.two_one_one,
                                                                              name=IOPWebservice.WS_NAME_GET_cpo_CDRs,
...
@ocpi_cpo_211.route("/cdrs<wildcard:path>", methods=['GET', 'PUT', 'POST', 'PATCH', 'DELETE'])
@requires_auth_api
def toserver_cpo_cdrs_error_no_ws(path):
