...
from app.api.utils.two_one_one.objects.versions_object import VersionsDetails
...
@ocpi_emsp_211.route("/", methods=['GET'])
@requires_auth_api
def toserver_get_emsp_versions():
    ...
    if not result_bool_req_headers or not result_bool_req_headers_ct_type:
        ...
        # Build the response
        version_details = VersionsDetails(version=Constants.TWO_DOT_ONE_DOT_ONE_CONSTANT, role=Constants.EMSP_CONSTANT)
        ocpi_response = OCPIResponse(data=version_details.serialize)
