...
@ocpi_emsp_211.route("/sessions/<country_code>/<party_id>/<session_id>", methods=['PUT'])
@requires_auth_api
def toserver_emsp_sessions_put(country_code, party_id, session_id):
    # Init result table
    result_tab = []
    ...
    if not result_bool_req_url or not result_bool_req_headers:
        ...
        result_bool_req_body, result_tab_req_body = request_body.schema_validator(self=request_body,
                                                                                  path_to_schema="app/api/utils/two_one_one/jsonschema/objects/OCPI_V211_Object_Sessions.json")
        ...
        if not result_bool_req_body:
            ...
            test_response.body = OCPIResponse.response_error(
                message="Some errors occurred => " + ','.join(v.__str__() for v in result_tab),
                error_code=2000)
        elif session_id != test_request.body.get('id'):
    ...
    # Recuperation du WS
    iop_ws = IOPWebserviceService.get_iop_webservice_by_ocpiversion_name_type(ocpiversion=OCPIVersion.two_one_one, name=IOPWebservice.WS_NAME_PUT_emsp_Sessions, ws_type=IOPWebservice.TYPE_TOSERVER)
...
@ocpi_emsp_211.route("/sessions/<country_code>/<party_id>/<session_id>", methods=['PATCH'])
@requires_auth_api
def toserver_emsp_sessions_patch(country_code, party_id, session_id):
    # Init result table
    result_tab = []
    test_request = TestRequest(url=request.url, headers=request.headers, body=request.json, tests=None)
    test_response = TestResponse(headers=None, tests=None, body=None, http_status_code=status.HTTP_200_OK)
    ...
    if not result_bool_req_url or not result_bool_req_headers:
        ...
        request_body = BodyValidator(test_request.body)
        result_bool_req_body, result_tab_req_body = request_body.schema_validator(self=request_body,
                                                                                  path_to_schema="app/api/utils/two_one_one/jsonschema/flows/sessions/OCPI_V211_ToServer_patch_emsp_sessions.json")
        result_tab.extend(result_tab_req_body)
        if not result_bool_req_body:
            test_request.tests = result_tab
            test_response.body = OCPIResponse.response_error(
                message="Some errors occurred => " + ','.join(v.__str__() for v in result_tab),
                error_code=2000)
        elif test_request.body.get('id') is not None and session_id != test_request.body.get('id'):
            result_tab.append(
...
@ocpi_emsp_211.route("/sessions/<country_code>/<party_id>/<session_id>", methods=['GET'])
@requires_auth_api
def toserver_emsp_sessions_get(country_code, party_id, session_id):
    # Init result table
    result_tab = []
    test_request = TestRequest(url=request.url, headers=request.headers, body=request.json, tests=None)
    test_response = TestResponse(headers=None, tests=None, body=None, http_status_code=status.HTTP_200_OK)
    ...
    if not result_bool_req_url or not result_bool_req_headers or not result_bool_req_headers_ct_type:
        ...
        # Build the response
        ocpi_response = None
        get_sessions_found_parameter = ParameterService.get_parameters_by_key_userid(Parameter.GET_SESSIONS_FOUND, g.user_id)
        # if Server must return not found error
        if get_sessions_found_parameter is not None and get_sessions_found_parameter.value == Constants.NO_CONSTANT:
            test_response.http_status_code = status.HTTP_404_NOT_FOUND
            ocpi_response = OCPIResponse(data=None)
        else:
            sess = Sessions(session_id=session_id, country_code=country_code, party_id=party_id)
            ocpi_response = OCPIResponse(data=sess.serialize)
        test_response.body = ocpi_response.response_success(self=ocpi_response,
                                                            message=','.join(v.__str__() for v in result_tab))
    ...
    test_report = Report(test_request, test_response)
    # Recuperation du WS
    iop_ws = IOPWebserviceService.get_iop_webservice_by_ocpiversion_name_type(ocpiversion=OCPIVersion.two_one_one, name=IOPWebservice.WS_NAME_GET_emsp_Sessions, ws_type=IOPWebservice.TYPE_TOSERVER)
    # Enregistrement du rapport de test
    TestReportService.save_test_report(TestReport(date_report=func.now(), report=test_report.serialize, status=test_report.get_status(test_report.request), web_service_id=iop_ws.webservice_id, user_id=g.user_id))
    return make_response(test_response.body, test_response.http_status_code)


@ocpi_emsp_211.route("/sessions<wildcard:path>", methods=['PATCH', 'PUT', 'DELETE', 'GET', 'POST'])
def toserver_emsp_sessions_error_no_ws(path):
