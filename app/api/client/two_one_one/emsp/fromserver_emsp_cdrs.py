...
@fromserver_emsp_211.route("/cdrs/GET_cpo_Cdrs", methods=['POST'])
@requires_auth
def fromserver_get_cpo_cdrs():
    ...
    # CREATE URL
    base_url = TestReportService.create_base_url(Constants.TWO_ONE_ONE_CONSTANT, Constants.CPO_CONSTANT,
    ...
    if result_bool_http_status_code and result_bool_res_headers_ct_type and result_bool_res_headers_pagination:
        ...
        if result_bool_json_body:
            ...
            result_bool_req_body, result_tab_req_body = response_body.schema_validator(self=response_body,
                                                                                       path_to_schema="app/api/utils/two_one_one/jsonschema/response/cdrs/OCPI_V211_FromServer_get_cpo_cdrs.json")
...
@fromserver_emsp_211.route("/cdrs<wildcard:path>", methods=['PATCH', 'PUT', 'DELETE', 'GET', 'POST'])
@requires_auth
def fromserver_emsp_cdrs_error_no_ws(path):
