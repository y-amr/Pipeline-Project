...
@fromserver_emsp_211.route("/tariffs/GET_cpo_Tariffs", methods=['POST'])
@requires_auth
def fromserver_get_cpo_tariffs():
    ...
    test_request.url = ParameterService.get_parameters_by_key_userid(Parameter.ENDPOINT,
                                                                     session_user_id).value + Constants.TO_CPO_TARIFFS_211_ENDPOINT_CONSTANT + pg_param.build_uri_pagination()
    # GET the URL
    if ParameterService.get_parameters_by_key_userid(Parameter.ENDPOINT_211_CPO_TARIFFS, session_user_id):
        test_request.url = ParameterService.get_parameters_by_key_userid(Parameter.ENDPOINT_211_CPO_TARIFFS,
        ...
        test_request.url = user_endpoint + Constants.TO_CPO_TARIFFS_211_ENDPOINT_CONSTANT + pg_param.build_uri_pagination()
    ...
    if result_bool_http_status_code and result_bool_res_headers_ct_type and result_bool_res_headers_pagination:
        ...
        if result_bool_json_body:
            ...
            result_bool_req_body, result_tab_req_body = response_body.schema_validator(self=response_body,
                                                                                       path_to_schema="app/api/utils/two_one_one/jsonschema/response/tariffs/OCPI_V211_FromServer_get_cpo_tariffs.json")
...
@fromserver_emsp_211.route("/tariffs<wildcard:path>", methods=['PATCH', 'PUT', 'DELETE', 'GET', 'POST'])
@requires_auth
def fromserver_emsp_tariffs_error_no_ws(path):
