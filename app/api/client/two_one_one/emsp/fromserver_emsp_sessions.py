...
@fromserver_emsp_211.route("/sessions/GET_cpo_Sessions", methods=['POST'])
@requires_auth
def fromserver_get_cpo_sessions():
    ...
    if ParameterService.get_parameters_by_key_userid(Parameter.ENDPOINT_211_CPO_LOCATIONS, session_user_id):
        ...
        test_request.url = user_endpoint + Constants.TO_CPO_SESSIONS_211_ENDPOINT_CONSTANT + pg_param.build_uri_pagination()
    ...
    if result_bool_http_status_code and result_bool_res_headers_ct_type and result_bool_res_headers_pagination:
        ...
        if result_bool_json_body:
            ...
            result_bool_req_body, result_tab_req_body = response_body.schema_validator(self=response_body,
                                                                                       path_to_schema="app/api/utils/two_one_one/jsonschema/response/sessions/OCPI_V211_FromServer_get_cpo_sessions.json")
    ...
    iop_ws = IOPWebserviceService.get_iop_webservice_by_ocpiversion_name_type(ocpiversion=OCPIVersion.two_one_one,
                                                                              name=IOPWebservice.WS_NAME_GET_cpo_Sessions,
...
@fromserver_emsp_211.route("/sessions<wildcard:path>", methods=['PATCH', 'PUT', 'DELETE', 'GET', 'POST'])
@requires_auth
def fromserver_emsp_sessions_error_no_ws(path):
