...
@fromserver_cpo_211.route("/versions/GET_emsp_Versions", methods=['POST'])
@requires_auth
def fromserver_get_emsp_versions():
    ...
    # GET the URL
    if ParameterService.get_parameters_by_key_userid(Parameter.ENDPOINT_211_EMSP_VERSIONS, session_user_id):
        test_request.url = ParameterService.get_parameters_by_key_userid(Parameter.ENDPOINT_211_EMSP_VERSIONS,
        ...
        test_request.url = user_endpoint + Constants.TO_EMSP_VERSIONS_ENDPOINT_CONSTANT
...
@fromserver_cpo_211.route("/versions/GET_emsp_Versions-details", methods=['POST'])
@requires_auth
def fromserver_get_emsp_versions_details():
    session_user_id = session['profile'].get('user_id')
    # creation de la requete a envoyer
    # Init result table
    result_tab = []
    ...
    test_request.url = ParameterService.get_parameters_by_key_userid(Parameter.ENDPOINT,
                                                                     session_user_id).value + Constants.TO_EMSP_VERSIONS_DETAILS_211_ENDPOINT_CONSTANT
    ...
    # Recuperation du WS
    iop_ws = IOPWebserviceService.get_iop_webservice_by_ocpiversion_name_type(ocpiversion=OCPIVersion.two_one_one,
                                                                              name=IOPWebservice.WS_NAME_GET_emsp_version_details,
                                                                              ws_type=IOPWebservice.TYPE_FROMSERVER
                                                                              )
    # Enregistrement du rapport de test
    TestReportService.save_test_report(TestReport(date_report=func.now(), report=test_report.serialize,
...
@fromserver_cpo_211.route("/versions<wildcard:path>", methods=['PATCH', 'PUT', 'DELETE', 'GET', 'POST'])
@requires_auth
def fromserver_cpo_versions_error_no_ws(path):
