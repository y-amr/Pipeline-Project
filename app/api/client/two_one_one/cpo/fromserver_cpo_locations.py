...
@fromserver_cpo_211.route("/locations/PATCH_emsp_Locations", methods=['POST'])
@requires_auth
def fromserver_patch_emsp_locations():
    ...
    try:
        ...
        return "Please fill a correct json request (JSON bad format)"

    # Check location_id consistency
    body_location_id = test_request.body.get("id")
    form_location_id = request.get_json().get("location_id")
    if form_location_id is None or form_location_id == '':
        form_location_id = Constants.DEFAULT_ID_CONSTANT
    if body_location_id is not None and body_location_id != form_location_id:
        return "The 'location.id' in the body doesn't match the one in the URL"
    ...
    base_url = TestReportService.create_base_url(Constants.TWO_ONE_ONE_CONSTANT, Constants.EMSP_CONSTANT,
                                                 Constants.LOCATIONS_CONSTANT, session_user_id)
    test_request.url = base_url + Constants.SLASH_CONSTANT + server_country_code + Constants.SLASH_CONSTANT + server_party_id + Constants.SLASH_CONSTANT + form_location_id
...
@fromserver_cpo_211.route("/locations/PUT_emsp_Locations", methods=['POST'])
@requires_auth
def fromserver_put_emsp_locations():
    session_user_id = session['profile'].get('user_id')
    # creation de la requete a envoyer
    # Init result table
    result_tab = []
    ...
    if form_location_id is None or form_location_id == '':
        form_location_id = Constants.DEFAULT_ID_CONSTANT
    if body_location_id is None or body_location_id != form_location_id:
        return "The 'location.id' in the body doesn't match the one in the URL"
    server_country_code = ParameterService.get_parameters_by_key_userid(Parameter.SERVER_COUNTRY_CODE,
                                                                        session_user_id).value
    server_party_id = ParameterService.get_parameters_by_key_userid(Parameter.SERVER_PARTY_ID, session_user_id).value
    ...
    # Recuperation du WS
    iop_ws = IOPWebserviceService.get_iop_webservice_by_ocpiversion_name_type(ocpiversion=OCPIVersion.two_one_one,
                                                                              name=IOPWebservice.WS_NAME_PUT_emsp_Locations,
                                                                              ws_type=IOPWebservice.TYPE_FROMSERVER
                                                                              )
    # Enregistrement du rapport de test
    TestReportService.save_test_report(TestReport(date_report=func.now(), report=test_report.serialize,
...
@fromserver_cpo_211.route("/locations/GET_emsp_Locations", methods=['POST'])
@requires_auth
def fromserver_get_emsp_locations():
    session_user_id = session['profile'].get('user_id')
    # creation de la requete a envoyer
    # Init result table
    result_tab = []
    ...
    if not test_response.http_status_code == status.HTTP_404_NOT_FOUND:
        ...
        if result_bool_http_status_code and result_bool_res_headers_ct_type:
            ...
            if result_bool_json_body:
                ...
                response_body = BodyValidator(test_response.body)
                result_bool_req_body, result_tab_req_body = response_body.schema_validator(self=response_body,
                                                                                           path_to_schema="app/api/utils/two_one_one/jsonschema/response/locations/OCPI_V211_FromServer_get_emsp_locations.json")
                result_tab.extend(result_tab_req_body)
                if result_bool_req_body:
                    if form_location_id != test_response.body.get('data').get('id'):
                        result_tab.append(Test(level=Test.ERROR,
                                               text="The 'location.id' returned doesn't match the Location requested"))
    ...
    # Recuperation du WS
    iop_ws = IOPWebserviceService.get_iop_webservice_by_ocpiversion_name_type(ocpiversion=OCPIVersion.two_one_one,
                                                                              name=IOPWebservice.WS_NAME_GET_emsp_Locations,
                                                                              ws_type=IOPWebservice.TYPE_FROMSERVER
                                                                              )
    # Enregistrement du rapport de test
    TestReportService.save_test_report(TestReport(date_report=func.now(), report=test_report.serialize,
...
@fromserver_cpo_211.route("/locations<wildcard:path>", methods=['PATCH', 'PUT', 'DELETE', 'GET', 'POST'])
@requires_auth
def fromserver_cpo_locations_error_no_ws(path):
